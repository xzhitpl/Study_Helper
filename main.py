...
class Page:
    def __init__(self, path, surface, pages, cut_, cut_sec):
        ...
        self.rects = []
        pos = []
        ignore = ["高一生物试题", "步步高", "题组", "能力", "第八章", "答案见", "年高考", "专题强化", "选择题",
                  "非选择题",
                  "易错", "一、", "二、", "致错", "应用", "第1页", "第2页", "第3页", "第4页", "第5页", "第6页", "第7页",
                  "第8页", "智能作业", "第七章", "第六章", "第五章", "第四章", "第三章", "第二章", "第一章"]
        for _i in result:
            if len(_i["words"]) > 2:
                go = True
                for j in ignore:
...
def err_handler(path, sub, cut_, cut_sec, erase_handwriting):
    ...
    try:
        ...
        with open("./error.log", "a", encoding="UTF-8") as f_:
            f_.write(err + "\n" + "#" * 60 + "\n\n")
        return err
...
def check_updates(show):
    ...
    try:
        ...
        if show:
            return 1, "检查更新失败", "检查更新失败，请检查您的网络连接！"
        else:
            return -1,
...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def setup_normal_thread(self, func):
        self.thread_ = Thread(func)
        self.thread_.signal_tuple.connect(self.normal_thread_finished)
        self.thread_.start()

    @pyqtSlot(tuple)
    def normal_thread_finished(self, item):
        if isinstance(item[0], tuple):
            if item[0][0] == 1:
                QMessageBox.critical(self, item[0][1], item[0][2])
            elif item[0][0] == 0:
    ...
    def contact_us(self):
        class Contact(QDialog, contact.Ui_Dialog):
            def __init__(self, parent=self):
                QDialog.__init__(self, parent)
                self.setupUi(self)
                self.accepted.connect(lambda: feedback.send(self.plainTextEdit.toPlainText()))
...
def except_hook(cls, exception, _traceback):
    ...
    with open("./error.log", "a", encoding="UTF-8") as f_:
        f_.write(err + "\n" + "#" * 60 + "\n\n")
    ctypes.windll.user32.MessageBoxW(0, f"错误信息：\n{err}这是一个出乎我们意料的错误，请反馈给我们！", "程序运行出错",
                                     0x00000010)
